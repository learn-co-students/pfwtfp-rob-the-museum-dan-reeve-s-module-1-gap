class Museum
  attr_accessor :galleries, :artists
  ...
  def new_from_csv(museum_csv_file)
    file = File.dirname(File.dirname(File.expand_path(__FILE__))) + "/" + museum_csv_file
    ...
    rows.each do |row|
      ...
      gallery = @galleries.find {|gallery| gallery.name == data[1]} ? @galleries.find {|gallery| gallery.name == data[1]} : Gallery.new(data[1])
